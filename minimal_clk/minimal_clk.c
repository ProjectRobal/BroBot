...
static PyObject *clock_set(PyObject *self, PyObject *args) {
   /* Do your stuff here. */
   ...
   freq=PyFloat_AsDouble(args);

   if ((freq < 4687.5) || (freq > 500e6)) { return PyLong_FromLong(-2);}
   ...
   for (i=0; i<CLK_SRCS; i++)
      ...
      if (gpioInitialise() < 0) return PyLong_FromLong(-3);
...
static PyObject *clock_off(PyObject *self, PyObject *args) {
...
static PyMethodDef module_methods[] = {
   { "clock_on", (PyCFunction)clock_set, METH_O, NULL },
   { "clock_off", (PyCFunction)clock_off, METH_NOARGS, NULL },
   { NULL, NULL, 0, NULL }
};

static struct PyModuleDef _clock_module=
{
...
PyMODINIT_FUNC PyInit_clock() {
   return PyModule_Create(&_clock_module);
}
